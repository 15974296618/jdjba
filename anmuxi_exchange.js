...
let helpnum = 1;
let waitTime = 300;//请求延迟
let exchangeTime = 50;//请求次数
$.prizeId=1;//奖品id  1 安慕希  2 护发素 3 小酒 4 毛毯 5 积木 6 红酒
...
async function run() {
  try {
    ...
    for (let j = 0; j <= exchangeTime; j++) {
      await takePostRequest('兑换');
      await $.wait(waitTime)
    }
  ...
  switch (type) {
    case 'isvObfuscator':
      ...
      case '报名':
        ...
        break;
      case '兑换':
        url = `${domain}/dingzhi/bigname/memberday/exchange`;
        body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&prizeId=${$.prizeId}`
        break;
      case 'info':
