...
[task_local]
#城城领现金
0 0-23/5,22 * 10 * jd_city_help.js, tag=城城领现金, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
...
let inviteCodes = [
 'QIPmyO2iSQqne8_WW9R_mrzn0ZgTGAXMsSoAVGYGaQ','GIfnzeqtQw--doOTF9c3msQ1C1cHz4w-tgi4_bbe-bzlO6bQ','RtGKzuSkSF6gftbPENw306xo665jFtO-LBZLx3O3PxVtvnXtrw'
]
$.shareCodesArr = [];
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      await getUA()
      for (let i = 0; i < inviteCodes.length && true; ++i) {
        console.log(`\n开始助力 【${inviteCodes[i]}】`)
        let res = await getInfo(inviteCodes[i])
        if (res && res['data'] && res['data']['bizCode'] === 0) {
          ...
          if (res['data']['result']['toasts'] && res['data']['result']['toasts'][0]) {
            console.log(`助力 【${inviteCodes[i]}】:${res.data.result.toasts[0].msg}`)
          }
        }
